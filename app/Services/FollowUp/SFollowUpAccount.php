...
class SFollowUpAccount implements IFollowUp
{
    ...
    )
    {
        ...
        $this->accountFollowupPhoto = $accountFollowupPhoto;
    ...
    public function getListFollowUp($id, $start_date, $end_date, $start, $length, $order)
    {
        ...
        $follow_up = $follow_up->where('sales_networking_id', $id)
                               ->whereBetween(DB::raw("TO_DATE(visit_date, 'DD-MM-YYYY')"), [$start_date, $end_date]);
        ...
        foreach ($follow_up as $value) {
            $value->total_followup_product = $this->getTotalFollowUpProduct($value->sales_networking_visit_id);
        }
    ...
    {
        $photos = $this->accountFollowupPhoto->where('sales_networking_visit_id', $id)->where('st', 1)->get();
    ...
    {
        return $this->accountFollowupProduct->where('sales_networking_visit_id', $id)->where('merk_hpl_id', 1)->count();
    }

    public function getSalesDetailById($id)
    {
        $product = $this->accountFollowupProduct
                        ->with([
                                'product_carta' => function($q) { $q->select('sales_networking_visit_product_id', 'product_id', 'quantity', 'points'); },
                                'product_carta.master_product' => function($q) { $q->select('product_id', 'product_code', 'product_alias', 'sales_material_group3_desc'); }
                            ])
                        ->where('sales_networking_visit_id', $id)
                        ->where('merk_hpl_id', 1)
                        ->first();
    ...
    public function followupHistoryJoin($id, $start_date, $end_date, $start, $length, $order)
    {
        $query = "
            ...
            LEFT JOIN mcs_sales_networking d on a.lead_id=d.lead_id
            WHERE d.sales_networking_id = ?
            AND a.st=1
