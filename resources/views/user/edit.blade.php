                ...
                <div class="card shadow mt-5 mb-5">
                    <form class="form-basic needs-validation" novalidate method="post" role="form" action="{{ route('user.edit.post') }}" enctype="multipart/form-data" id="form-create-lead">
                            ...
                            <div class="row">
                                <div class="col-lg-4 col-md-6 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="nik" name="nik" class="form-control" autocomplete="off" required autofocus value="{{ str_pad($user['nik'], 9, '0', STR_PAD_LEFT) }}" readonly>
                            ...
                            <div class="row">
                                <div class="col-lg-4 col-md-6 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="name" name="name" class="form-control" autocomplete="off" required autofocus value="{{ $user['name'] }}" readonly>
                                        <div class="invalid-feedback">Please fill out this field.</div>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <input type="email" id="email" name="email" class="form-control" autocomplete="off" value="{{ $user['email'] }}" readonly> 
                                        <div class="invalid-feedback">Please fill out this field.</div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="nik_atasan" name="nik_atasan" class="form-control select-type-2">
                                            ...
                                            @foreach ($user_list as $item)
                                                <option value="{{ $item['nik'] }}" {{ ($item['nik']==$user['nik_atasan']) ? 'selected' : '' }}>{{ $item['nik'] . ' | '. $item['name'] }}</option> 
                                            @endforeach
                                        </select>
                                        <div class="invalid-feedback">Please fill out this field.</div>
                                    </div>
                                ...
                                <div class="col-lg-8">
                                    <div class="form-group">
                                        ...
                                        <select id="nik_atasan2" name="nik_atasan2" class="form-control select-type-2">
                                            <option value=""></option>
                                            @foreach ($user_list as $item)
                                                <option value="{{ $item['nik'] }}" {{ ($item['nik']==$user['nik_atasan2']) ? 'selected' : '' }}>{{ $item['nik'] . ' | '. $item['name'] }}</option> 
                                            @endforeach
                                        </select>
                                        <div class="invalid-feedback">Please fill out this field.</div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="role" name="role" class="form-control select-type-1" required>
                                            ...
                                            @foreach ($roles as $item)
                                                <option value="{{ $item->role_id }}" {{ ($item->role_id==$user['role_id']) ? 'selected' : '' }} >{{ $item->name }}</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <select id="teritory" name="teritory[]" class="form-control select-type-2" multiple required>
                                            ...
                                            @foreach ($teritory as $item)
                                                ...
                                                @if (count($user_teritory)>0)
                                                    @foreach ($user_teritory as $item2)
                                                        {{ $selected = '' }}
                                                        @if ($item->teritory_id==$item2->teritory_id)
                                                            {{ $selected = 'selected' }}
                                                            @break
                                                        @endif
                                                    @endforeach
                                                @endif
                                                <option value="{{ $item->teritory_id }}" {{ $selected }} >{{ $item->name }}</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="sales_org" name="sales_org" class="form-control select-type-1">
                                            ...
                                            @foreach ($sales_org as $item)
                                                <option value="{{ $item }}" {{ ($item==$user['sales_org']) ? 'selected' : '' }} >{{ $item }}</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-4 col-md-6 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <select id="status" name="status" class="form-control select-type-1" required>
                                            <option value="1" {{ ($user['st']==1) ? 'selected' : '' }}>Active</option>
                                            <option value="0" {{ ($user['st']==0) ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-footer text-center">
                            <div class="d-flex">
                                <input type="hidden" id="nik" name="nik" value="{{ str_pad($user['nik'], 9, '0', STR_PAD_LEFT) }}">
